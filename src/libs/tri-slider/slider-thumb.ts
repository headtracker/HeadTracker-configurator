 ...
 */
@Component({
  selector: 'tri-slider-visual-thumb',
  ...
  providers: [{ provide: TRI_SLIDER_VISUAL_THUMB, useExisting: TriSliderVisualThumb }],
  standalone: true,
  imports: [MatRipple],
})
export class TriSliderVisualThumb implements _TriSliderVisualThumb, AfterViewInit, OnDestroy {
  readonly _cdr = inject(ChangeDetectorRef);
  private readonly _ngZone = inject(NgZone);
  private _slider = inject<_TriSlider>(TRI_SLIDER);
  ...
  private _sliderInput!: _TriSliderThumb;
  ...
  private _showRipple(animation: RippleAnimationConfig, ignoreGlobalRippleConfig?: boolean): RippleRef | undefined {
    ...
    if (this._slider._isRange) {
      switch (this.thumbPosition) {
        case _TriThumb.START:
          this._slider._getThumb(_TriThumb.END)._showValueIndicator();
          this._slider._getThumb(_TriThumb.MID)._showValueIndicator();
          break;
        case _TriThumb.END:
          this._slider._getThumb(_TriThumb.START)._showValueIndicator();
          this._slider._getThumb(_TriThumb.MID)._showValueIndicator();
          break;
        case _TriThumb.MID:
          this._slider._getThumb(_TriThumb.START)._showValueIndicator();
  ...
  _getSiblings(): _TriSliderVisualThumb[] {
    switch (this.thumbPosition) {
      case _TriThumb.START:
        return [this._slider._getThumb(_TriThumb.END), this._slider._getThumb(_TriThumb.MID)];
      case _TriThumb.END:
        return [this._slider._getThumb(_TriThumb.START), this._slider._getThumb(_TriThumb.MID)];
      case _TriThumb.MID:
        return [this._slider._getThumb(_TriThumb.START), this._slider._getThumb(_TriThumb.END)];
    }
  }
