...
import { NullFlash } from '@bossa-web/src/NullFlash';
...
export class Device {
  ...
  async create(): Promise<void> {
    let flashPtr: Flash | null = null;

    // If the bootloader supports automatic chip identification...
    if (this._samba.canIdentifyChip) {
      let id = await this._samba.identifyChip();

      if (id == 'nRF52840-QIAA') {
        this._family = Family.FAMILY_NRF52;
        flashPtr = new NullFlash(this._samba, 'nRF52840-QIAA', 256, 4096, 0x00000000, 0x00000000);
      } else {
  ...
  async reset(): Promise<void> {
    try {
      if (this._samba.canReset) {
        await this._samba.reset();
        return;
      }
