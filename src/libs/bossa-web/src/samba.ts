...
import { sleep, toByteArray, Uint8Buffer } from './util';
...
export class SamBA {
  ...
  private _canIdentifyChip: boolean = false;
  ...
  async checksumBuffer(start_addr: number, size: number): Promise<number> {
    if (!this._canWriteBuffer) throw new SamBAError('Cannot write buffer');

    if (size > this.checksumBufferSize()) throw new SamBAError('Size too large for checksum buffer');
    ...
    let result = await this.sendCommand(
      'Z' + this.hex(start_addr) + ',' + this.hex(size),
      12,
      undefined,
      0,
      TIMEOUT_LONG,
  ...
  async chipErase(start_addr: number): Promise<void> {
    if (!this._canChipErase) throw new SamBAError('Chip erase not supported');

    if (this.options.debug) this.options.logger.debug('chipErase(start_addr=0x', this.hex(start_addr), ')');

    let result = await this.sendCommand('X' + this.hex(start_addr), 3, undefined, 0, TIMEOUT_LONG);

    if (!result || result[0] != 0x58 /* 'X' */) throw new SamBAError("Board response for 'X' command wrong");
  }
  ...
  async writeBuffer(src_addr: number, dst_addr: number, size: number) {
    ...
    if (!result || result[0] != 0x59 /* 'Y' */) throw new SamBAError("Board response for 'Y' command wrong");
  ...
  async readByte(addr: number): Promise<number> {
    if (this.options.debug) this.options.logger.debug('readByte(addr=0x', this.hex(addr), ')');
    ...
    if (result) {
      ...
      if (this.options.debug) this.options.logger.debug('readByte(addr=0x', this.hex(addr), ')=0x', this.hex(value, 2));
  ...
  async readWord(addr: number): Promise<number> {
    if (this.options.debug) this.options.logger.debug(`readWord(addr=0x${this.hex(addr)})`);
    ...
    if (result) {
      ...
      if (this.options.debug) this.options.logger.debug('readByte(addr=0x', this.hex(addr), ')=0x', this.hex(value, 8));
  ...
  async write(addr: number, buffer: Uint8Array, size: number = buffer.length): Promise<void> {
    if (this.options.debug) this.options.logger.debug('write(addr=0x', this.hex(addr), ',size=0x', this.hex(size), ')');
  ...
  async read(addr: number, buffer: Uint8Array, size: number): Promise<void> {
    if (this.options.debug) this.options.logger.debug('read(addr=0x', this.hex(addr), ',size=0x', this.hex(size), ')');
    ...
    while (size > 0) {
      ...
      if (this._readBufferSize > 0 && size > this._readBufferSize) chunk = this._readBufferSize;
      ...
      } else throw new SamBAError('Reading binary');
  ...
  async go(addr: number): Promise<void> {
    if (this.options.debug) this.options.logger.debug('go(addr=0x', this.hex(addr), ')');
  ...
  private async _connect() {
    ...
    this.readLoopPromise = (async () => {
      this.readLoop().catch((reason: any) => {
    ...
    let extIndex = version.indexOf('[Arduino:');

    if (this.options.debug) this.options.logger.debug('Version-Info from bootloader: ' + version);
  ...
  async identifyChip() {
    let buffer = await this.sendCommand('I', 256);

    if (buffer) {
      return this.decodeResponse(buffer).trim();
    }
  ...
  async reset() {
    if (!this._canReset) {
      throw new SamBAError('Reset not supported');
    }

    let buffer = await this.sendCommand('K', 2);

    if (buffer) {
      return this.decodeResponse(buffer);
    }

    throw new SamBAError('Reset failed');
  }
  ...
  private async sendCommand(
    ...
    if (responseSize > 0) return await this.readBuffer(timeout, responseSize);
    else return null;
  }
  ...
  private async readBuffer(
    timeout: number = DEFAULT_TIMEOUT,
    responseSize: number | undefined = undefined,
  ): Promise<Uint8Array | null> {
    let reply: number[] = [];
    ...
    if (this.serialPort.readable) {
      ...
      try {
        while (!this.closed) {
          ...
          if (value) {
            if (this.options.trace) this.logger.debug(`Received ${value}`);
